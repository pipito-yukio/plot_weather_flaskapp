...
def loadMonthDataFrame(
        ...
        logger: Optional[logging.Logger] = None, logger_debug: bool = False) -> Optional[pd.DataFrame]:
    """
    外気温・室内湿度・気圧の月間データを取得する ※室内気温を除く
    :param dao: WeatherDao
    :param device_name: デバイス名
    :param year_month: 今年の年月
    :param logger: アプリロガー
    :param logger_debug: デバック有効フラグ (デフォルト無効)
    :return: レコードが存在する場合は DataFrame、存在しない場合は None
    """
    rec_count: int
    csv_buffer: StringIO
...
def getDataFramesWithYearMonth(
        conn: connection, device_name: str, curr_year_month,
        logger=None, logger_debug=False
) -> Tuple[Optional[DataFrame], Optional[DataFrame], str]:
    """
    指定された年月の気象データのDataFrameを取得する
    :param conn: psycopg2.connection
    :param device_name: デバイス名
    :param curr_year_month: 今年の年月
    :param logger: アプリロガー
    :param logger_debug: デバック有効フラグ (デフォルト無効)
    :return: レコードあり(今年DataFrame, 前年のDataFrame, 前年月)
     レコードなし (None, None, 前年月)
    """
    dao = WeatherPrevCompDao(conn, logger=logger)
    try:
        # 今年の年月テータ取得
        df_curr: Optional[DataFrame] = loadMonthDataFrame(
            dao, device_name, curr_year_month,
            logger=logger, logger_debug=logger_debug)
        # 前年の年月テータ取得
        # 前年計算
        prev_year_month: str = toPreviousYearMonth(curr_year_month)
        df_prev: Optional[DataFrame] = loadMonthDataFrame(
            dao, device_name, prev_year_month,
            logger=logger, logger_debug=logger_debug)
        return df_curr, df_prev, prev_year_month
    except Exception as err:
        logger.warning(err)
        raise err


def make_graph(df_curr: DataFrame, df_prev: DataFrame,
...
def gen_plot_image(
        conn: connection, device_name: str, year_month: str, logger=None
    ...
    :param year_month: 今年の年月
    :param logger: app_logger
    :return: データがある場合は(今年の年月データ件数, 比較年月データ画像(base64エンコード文字列))、
    ない場合は (0, None)
    """
    ...
    df_curr, df_prev, prev_year_month = getDataFramesWithYearMonth(
        conn, device_name, year_month,
        logger=logger, logger_debug=logger_debug
    )
    if logger is not None and logger_debug:
        logger.debug(f"df_curr: {df_curr}, df_prev: {df_prev}")
    # 件数チェック
    ...
    # グラフ生成
    fig: Figure = make_graph(df_curr, df_prev,
    ...
    # 件数と画像
    return df_curr.shape[0], img_src
